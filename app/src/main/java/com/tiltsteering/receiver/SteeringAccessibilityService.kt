...
class SteeringAccessibilityService : AccessibilityService() {
    companion object {
        ...
        const val LEFT_X  = 180f;  const val LEFT_Y  = 900f
        const val RIGHT_X = 380f;  const val RIGHT_Y = 900f
        const val ACCEL_X = 980f;  const val ACCEL_Y = 850f
    ...
    fun handleTilt(tilt: Float) {
        val dir = when { tilt > DEADZONE -> -1; tilt < -DEADZONE -> 1; else -> 0 }
        if (dir == lastDir && abs(tilt - lastTilt) < THRESHOLD) return
        lastDir = dir; lastTilt = tilt
        when (dir) {
    ...
    private fun hold(x: Float, y: Float) {
        if (busy) return
        val stroke = GestureDescription.StrokeDescription(Path().apply { moveTo(x,y) }, 0, HOLD_MS, true)
        busy = true
        dispatchGesture(GestureDescription.Builder().addStroke(stroke).build(), object: GestureResultCallback() {
            override fun onCompleted(g: GestureDescription) {
                busy = false
                val cont = (lastDir == -1 && x == LEFT_X) || (lastDir == 1 && x == RIGHT_X) || (accelHeld && x == ACCEL_X)
                if (cont) hold(x, y)
            }
